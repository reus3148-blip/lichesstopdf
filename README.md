...
- 프런트엔드: `web/`의 정적 페이지
  ...
  - `/game` → `web/game.html` (게임 기보 인쇄)
- 백엔드: `api/`의 Vercel Serverless Function
  - `api/puzzles.py` → Neon Postgres에서 popularity=100 퍼즐을 조회
  - `api/study.py` → 스터디 PGN 또는 붙여넣은 게임 PGN을 인쇄용 HTML로 렌더 (`study_core.py` 공유)
...
- `--columns`: 가로·세로 칸 수 N. 페이지를 N×N 칸으로 나눠 배치합니다 (기본 2 → 페이지당 4개).
...
- `--layout study|game`: `study`는 코멘트가 붙는 N×N 그리드, `game`은 한 페이지 8개짜리 기보 시트입니다.
- `--html-only`: PDF 없이 HTML까지만 만듭니다.

각 카드는 고정 크기라 페이지가 N×N 칸으로 일정하게 나뉩니다. 메인라인 카드는 굵은 검정 테두리, 변화수는 깊이에 따라 테두리 스타일(실선·점선)과 회색 음영으로 구분돼 흑백 인쇄에서도 읽힙니다. PGN 코멘트는 다이어그램 아래에 붙고, `[%cal]`/`[%csl]` 주석은 보드 위 화살표로 그려집니다.

## 게임 기보 인쇄하기

한 게임 전체를 수마다 다이어그램으로 펼친 기보 시트로 만들 수 있습니다. 코멘트가 많은 스터디와 달리, 게임 PGN은 텍스트가 적으므로 `--layout game`은 보드를 빽빽하게 깔아 **A4 한 페이지에 8개**(2열 × 4행)씩 배치합니다. 보드에는 수 표기만 붙고, 게임 헤더는 대국자 이름으로 채워집니다.

```powershell
.\.venv\Scripts\python.exe .\make_study_pdf.py `
  --input .\data\sample_game.pgn `
  --layout game `
  --output .\output\game_sheet.pdf
```

한 PGN에 여러 게임이 들어 있으면 게임마다 새 페이지에서 시작합니다. 변화수가 있으면 회색 음영으로 구분해 함께 출력합니다.

공개 Lab에서는 `/game` 페이지(`web/game.html`)에 PGN을 붙여넣어 같은 시트를 만들 수 있습니다.
